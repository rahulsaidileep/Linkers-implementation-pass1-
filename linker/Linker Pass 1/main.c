...
void pass1(char PROGADDR_HEX[10]){
	int PROGADDR = (int)strtol(PROGADDR_HEX, NULL, 16); // to convert str to int
	CSADR = PROGADDR;
    ...
    while ((fgets(&line, &len, fp1)) != NULL) //fgets -reads up to n characters from the stream (file stream or standard input stream) to a string str 
    {
        int len = 0;
        char *p = strtok (line, delimit); //to split a string into a series of tokens based on a particular delimiter
        char *array[100];
